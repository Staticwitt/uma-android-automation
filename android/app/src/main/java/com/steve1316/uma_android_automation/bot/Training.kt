            ...
            TrainingConfig(
                ...
                statTargets =
                    if (disableStatTargets) {
                        ...
                        // Optional upstream stamina scaling on top of the fork's per-style bias. Default off, so existing targets are unchanged unless the user opts in.
                        if (enableRunningStyleStaminaAdjustment && campaign.trainee.bHasSetRunningStyle) {
                            applyRunningStyleStaminaFactor(biasedTargets, campaign.trainee.runningStyle)
                        } else {
                            biasedTargets
                        }
                    },
