...
class YoloDetector(private val context: Context) {
    ...
    private fun letterbox(bitmap: Bitmap): LetterboxInfo {
        ...
        // Scale the image and draw it centered onto the padded background. The scaled intermediate is recycled immediately -
        // it is drawn into the letterbox canvas and never used again, and detect() runs often enough per turn that waiting on
        // the garbage collector adds real memory pressure.
    ...
    fun detect(bitmap: Bitmap): List<Detection> {
        ...
        try {
            // Create input tensor and run inference. The input tensor owns native (off-heap) memory that the JVM garbage collector
            // never reclaims, so it must be closed explicitly - leaking it costs ~5 MB per detection call and progressively slows
            // every subsequent inference as native memory pressure mounts over a career.
