...
object DialogUtils {
    ...
    fun getTitle(imageUtils: CustomImageUtils, bitmap: Bitmap? = null): String? {
        ...
        // The title bar's gradient pattern repeats, so a template match can land anywhere along the
        // bar rather than strictly at its left edge. The title bar is horizontally centered on screen,
        // so reflect the detected edge around the center to always recover the true (smaller) left
        // edge candidate. This keeps the already-correct case unchanged while preventing a negative
        // width when the match lands right of center.
