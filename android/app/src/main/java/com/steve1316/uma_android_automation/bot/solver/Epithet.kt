...
object EpithetFilters {
    ...
    /** Graded plus Open-class races (G1/G2/G3/OP/Pre-OP). Mirrors [MilpSolver] grade-at-least-open checks. */
    private val GRADED_OR_OPEN: Set<RaceGrade> =
        setOf(RaceGrade.G1, RaceGrade.G2, RaceGrade.G3, RaceGrade.OP, RaceGrade.PRE_OP)

    /**
     * Shared filter predicate for epithet win-count matchers. Keep in sync with the TypeScript mirror in `src/lib/solver/scoring.ts`.
     *
     * @param race Race candidate to test.
     * @param filter Filter predicate from a win-count matcher.
     * @return True when every non-null / non-empty field of [filter] accepts [race].
     */
    fun raceMatchesFilter(race: RaceCandidate, filter: EpithetFilter): Boolean {
        if (filter.terrain != null && race.terrain != filter.terrain) return false
        if (filter.grade != null && race.grade != filter.grade) return false
        if (filter.gradeAtLeastOpen && race.grade !in GRADED_OR_OPEN) return false
        if (filter.gradedOnly && race.grade !in GRADED) return false
        ...
        if (filter.nameContainsCountry && !nameContainsCountry(race.name)) return false
        return true
    }
}
 ...
 * `scripts/data-scraper/main.py`. The scraper owns [bullets] and regenerates [matchers] via
 * `derive_matchers`; reward kind/amount, scenario restrictions, and prerequisite epithet names are
 * derived at runtime via [EpithetFilters].
 *
