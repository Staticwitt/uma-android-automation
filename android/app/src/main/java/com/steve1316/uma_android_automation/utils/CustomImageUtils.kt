...
import com.steve1316.uma_android_automation.components.IconUnityCupSpiritExplosionExtreme
...
class CustomImageUtils(context: Context, private val game: Game) : ImageUtils(context) {
     ...
     * @property numExtremeGaugesReady Number of facilities showing an Extreme Spirit Burst (available after a facility's regular
     *   burst has already been triggered there) - strictly better than a regular burst, so scored separately and higher.
     */
    data class SpiritGaugeResult(val numGaugesCanFill: Int, val numGaugesReadyToBurst: Int, val numExtremeGaugesReady: Int = 0)
    ...
    fun findTrainingFailureChance(sourceBitmap: Bitmap? = null, trainingSelectionLocation: Point? = null, tries: Int = 1): Int {
        ...
        for (i in 1..tries) {
            ...
            if (result == -1) {
                ...
                // Stop retrying once we have a valid read instead of burning the remaining attempts.
                break
            }
        }
    ...
    private fun findSupportFaceCircles(sourceBitmap: Bitmap): List<IntArray> {
        val regionX = SharedData.displayWidth - SharedData.displayWidth / 3
        val regionHeight = (SharedData.displayHeight * RAINBOW_SUPPORT_REGION_HEIGHT_FRACTION).toInt()
        val roi = createSafeBitmap(sourceBitmap, regionX, 0, SharedData.displayWidth / 3, regionHeight, "findSupportFaceCircles") ?: return emptyList()

        val rgbaMat = Mat()
        Utils.bitmapToMat(roi, rgbaMat)
        val grayMat = Mat()
        ...
        val scale = SharedData.displayWidth.toDouble() / 1080.0
        val circles = Mat()
    ...
    fun debugRainbowDetection(sourceBitmap: Bitmap? = null): String {
        ...
        val regionX = SharedData.displayWidth - SharedData.displayWidth / 3
        val annotatedBitmap =
            createSafeBitmap(bitmap, regionX, 0, SharedData.displayWidth / 3, (SharedData.displayHeight * RAINBOW_SUPPORT_REGION_HEIGHT_FRACTION).toInt(), "debugRainbowDetection annotate")
        val annotated =
            annotatedBitmap?.let {
                val mat = Mat()
    ...
    fun analyzeSpiritExplosionGauges(sourceBitmap: Bitmap? = null): SpiritGaugeResult? {
        ...
        // Find all Extreme Spirit Burst icons (purple, only appear after a facility's regular burst has already been triggered).
        val extremeSpiritExplosionIcons: ArrayList<Point> = IconUnityCupSpiritExplosionExtreme.findAll(this, sourceBitmap = currentBitmap, confidence = 0.9)
        ...
        if (finalLocation != null && finalSourceBitmap != null) {
            ...
            StatName.entries.forEachIndexed { index, statName ->
                ...
                if (text.lowercase().contains("max") || text.lowercase().contains("ax")) {
                    ...
                    result[statName] = if (manualStatCap > 0) manualStatCap else 1950
                } else {
                    try {
    ...
    fun detectScrollBar(bitmap: Bitmap? = null, region: BoundingBox? = null, minArea: Int? = null, maxArea: Int? = null, morphCloseKernelSize: Int = 10): Pair<BoundingBox?, BoundingBox?> {
        ...
        if (resolvedBitmap == null || resolvedBitmap.isRecycled || resolvedBitmap.width <= 0 || resolvedBitmap.height <= 0) {
            MessageLog.w(TAG, "[WARN] detectScrollBar:: Invalid or missing bitmap (region=$region).")
        ...
        val image = Mat()
        val hsvImage = Mat()
        var barMask: Mat? = null
        var thumbMask: Mat? = null

        try {
            Utils.bitmapToMat(resolvedBitmap, srcImage)
            ...
            fun detectFromMask(mask: Mat, minArea: Int, maxArea: Int, debugString: String = ""): BoundingBox? {
                val debugImage = Mat()
                Utils.bitmapToMat(resolvedBitmap, debugImage)
                ...
                for (cnt in contours) {
                    ...
                    if (rect.x <= 0 ||
                        rect.y <= 0 ||
                        rect.x + rect.width >= resolvedBitmap.width - 1 ||
                        rect.y + rect.height >= resolvedBitmap.height - 1
                    ) {
                        continue
                    }
            ...
            MessageLog.e(TAG, "[ERROR] detectScrollBar:: Failed to detect scrollbar: ${e.message}")
            return Pair(null, null)
        } finally {
            barMask?.release()
            thumbMask?.release()
