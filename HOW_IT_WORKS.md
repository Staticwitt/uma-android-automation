...
> [!TIP]
> **Parent Farming Mode.** Racing Settings includes a parent-farming preset for unattended inheritance runs. Enabling it turns on Smart Race Solver, keeps Force Racing and the in-game race agenda off, enables the fan-farming fallback, applies the "Fans + Epitaphs" solver mode with a lower per-race cost and lighter consecutive-race penalty, allows career completion after mandatory-race failure, relaxes stat targets, prioritizes skill hints, and sets inheritance **Spark Selection Strategy** to stat/aptitude mode. Character presets, aptitudes, target/forced epithets, and manual locks remain user-controlled in Smart Race Solver Settings.
...
- Apply **Character + Goal Bundles** on Racing Settings or Smart Race Solver Settings for one-tap parent setups (character preset, aptitudes, goal epithets, solver weights, and training distance bias). Bundles replace target epithets and clear manual locks; use **Parent Goal Presets** below to merge epithets into an existing list instead.
- Apply **Parent Goal Presets** to quickly add common parent-farming targets such as G1 / fan parents, Triple Crown, Triple Tiara, dirt, mile / sprint, medium / long, stayer, derby line, queen's race, turf all-rounder, senior finale, junior star, and skill-hint goals. Presets preserve existing picks and tune the target-epithet multiplier so selected targets outscore incidental epithets.
...
- Tune the **weights bundle** (`raceValue`, `epithetValue`, `targetEpithetMultiplier`, `statWeight`, `spWeight`, `hintWeight`, `consecutiveRacePenalty`, `summerPenalty`, `raceBonusPct`, `raceCostPct`, `minimumRaceGapTurns`, `aptitudeThreshold`, `includeOpAndPreOp`, `allowSummerRacing`). `minimumRaceGapTurns = 1` is the parent-farming default and prevents back-to-back solver-planned races.
